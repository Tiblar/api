<?php
namespace App\Service\SpamFilter\Lexer;

class Lexer
{
    const LEXER_NO_TOKENS = 'b8*no_tokens';

    private array $config = [ 'min_size'      => 3,
                        'max_size'      => 30,
                        'get_uris'      => true,
                        'get_html'      => true,
                        'get_bbcode'    => false,
    ...
    private array $regexp = [ 'raw_split' => '/[\s,\.\/"\:;\|<>\-_\[\]{}\+=\)\(\*\&\^%]+/',
                        'ip'        => '/([A-Za-z0-9\_\-\.]+)/',
        ...
        foreach ($config as $name=>$value) {
            switch ($name) {
                ...
                case 'get_bbcode':
                    ...
                    break;
                default:
                    throw new \Exception(Lexer::class . ": Unknown configuration key: "
                                         . "\"$name\"");
            }
        }
    }
     ...
     * @return array Returns a list of tokens
     * @throws \Exception
     */
    public function get_tokens(string $text)
    {
        ...
        if(is_string($text) === false){
            throw new \Exception("Text is not a string.");
        }
    ...
    private function is_valid(string $token)
    {
        ...
        if($this->config['allow_numbers'] === false && preg_match($this->regexp['numbers'], $token) > 0){
            return false;
        }
